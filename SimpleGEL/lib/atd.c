...
void (*interruptAction_f)(word*, word*);
...
bool atd_start_conversion(unsigned short port){
    bool res = false;
    if(port <= 1){
	byte prevState = _io_ports[M6812_ATD0CTL5 + port*DIST_AD];
...
void __attribute__((interrupt)) vi_atd(void){
    byte port = ((_io_ports[M6812_ATD0STAT0] && M6812B_SCF)? 0: 1);
...
void atd_get_last_result(word * result, word * n){
    unsigned short i=0;
    for(i=0; i < lastN; i++){
...
void atd_interruptAction(void (*f)(word*, word*), word * result, word * n){
    interruptAction_f = f;
    resultDest = result;
    nDest = n;
}
